    ...
    | In.Scan (f, n_exp, arr_exp, _, pos)
      => let val (n_type, n_dec) = checkExp ftab vtab n_exp
             ...
             val elem_type =
               case arr_type of
                  Array t => t
                | other => raise Error ("Scan: argument is not an array", pos)
             val (f', f_arg_type) = 
               case checkFunArg (f, vtab, ftab, pos) of
                   (f', res, [a1, a2]) =>
                   if a1 = a2 andalso a2 = res
...
(*
    | In.Filter (f, arr_exp, _, pos)
      => let val (arr_type, arr_dec) = checkExp ftab vtab arr_exp
             
              val elem_type =
                case arr_type of
                  Array t => t
                | other => raise Error ("Filter: argument is not an array", pos)
                ...
                case checkFunArg (f, vtab, ftab, pos) of
                  ...
                  | (_,  res) =>
                   raise Error ("Filter: incompatible function type of "
                                ^ In.ppFunArg 0 f ^ ":" ^ showFunType (res), pos)
